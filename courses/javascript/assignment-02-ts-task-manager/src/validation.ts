...
export function validateCategoryPriority(category: Category, priority: Priority): void {
...
export function validateTaskCore(input: TaskCore): TaskCore {
...
export function validateTaskUpdate(input: TaskUpdateInput): TaskUpdateInput {
  const updated: TaskUpdateInput = {};
...
export function validateFilters(raw: TaskFilters): TaskFilters {
...
export function validateSort(sort: TaskSort): TaskSort {
  const by = assertOneOf(sort.by, SORT_FIELDS, "sort field") as SortField;
  const direction = assertOneOf(
    sort.direction,
    ["asc", "desc"] as const,
    "sort direction"
  ) as SortDirection;
