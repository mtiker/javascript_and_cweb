...
import { parseRecurrenceRule, validateFilters, validateSearchQuery, validateSort, validateTaskCore, validateTaskUpdate } from "./validation.js";
...
export class TaskService {
    ...
    async queryTasks(options) {
        ...
        if (filters.dueBefore) {
            tasks = tasks.filter((task) => task.dueDate !== null && task.dueDate <= filters.dueBefore);
        }
        if (filters.tag) {
    ...
    createTaskCoreFromInput(raw) {
        return validateTaskCore({
            ...
            recurrence: parseRecurrenceRule(raw.recurrenceFrequency, raw.recurrenceInterval, raw.recurrenceEndDate)
        });
    }
    ...
    ensureDependenciesDone(tasks, dependencyIds) {
        ...
        if (pending.length > 0) {
            throw new ValidationError(`cannot mark as done. unresolved dependencies: ${pending.join(", ")}`);
        }
    }
    ...
    normalizeStoredTask(raw) {
        ...
        try {
            const recurrence = parseRecurrenceRule(record.recurrence?.frequency ?? "none", String(record.recurrence?.interval ?? 1), record.recurrence?.endDate ?? "");
