import { CATEGORY_PRIORITY_RULES, CATEGORY_VALUES, LIMITS, PRIORITY_VALUES, RECURRENCE_VALUES, SORT_FIELDS, STATUS_VALUES } from "./constants.js";
...
export function parseRecurrenceRule(frequencyRaw, intervalRaw, endDateRaw) {
    const frequency = assertOneOf(normalizeText(frequencyRaw) || "none", RECURRENCE_VALUES, "recurrence frequency");
...
export function validateCategoryPriority(category, priority) {
    ...
    if (!allowed.includes(priority)) {
        throw new ValidationError(`Category "${category}" does not allow priority "${priority}". Allowed: ${allowed.join(", ")}.`);
    }
}
export function validateTaskCore(input) {
    ...
    if (invalidTag) {
        throw new ValidationError(`tag "${invalidTag}" exceeds ${LIMITS.tagLength} characters.`);
    }
    if (input.dependencies.length > LIMITS.dependencies) {
        throw new ValidationError(`max ${LIMITS.dependencies} dependencies allowed for one task.`);
    }
