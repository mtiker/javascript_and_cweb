...
{
    private static readonly Assembly[] PublicApiAssemblies =
    [
        typeof(Program).Assembly,
        typeof(AccountController).Assembly,
        typeof(Modules.Gyms.Api.System.GymsController).Assembly,
        typeof(Modules.Memberships.Api.Tenant.MembersController).Assembly,
        typeof(Modules.Training.Api.Tenant.TrainingSessionsController).Assembly,
        typeof(Modules.Maintenance.Api.Tenant.MaintenanceTasksController).Assembly,
    ];

    private static IEnumerable<Type> DiscoverPublicApiControllers()
    {
        return PublicApiAssemblies
            .SelectMany(assembly => assembly.GetTypes())
            .Where(type =>
                type is { IsClass: true, IsAbstract: false } &&
                typeof(ControllerBase).IsAssignableFrom(type) &&
                (type.Namespace?.StartsWith("WebApp.ApiControllers", StringComparison.Ordinal) == true ||
                 (type.Namespace?.StartsWith("Modules.", StringComparison.Ordinal) == true &&
                  (type.Namespace.EndsWith(".Api", StringComparison.Ordinal) ||
                   type.Namespace.Contains(".Api.", StringComparison.Ordinal)))));
    }

    [Fact]
    public void PublicApiControllers_DeclareStandardProblemDetailsErrorMetadata()
    {
        var controllerTypes = DiscoverPublicApiControllers().ToArray();
    ...
    [Fact]
    public void PublicApiControllers_UseV1UrlSegmentApiVersioning()
    {
        var controllerTypes = DiscoverPublicApiControllers().ToArray();
        ...
        foreach (var controllerType in controllerTypes)
        {
            Assert.True(HasApiControllerMetadata(controllerType), $"{controllerType.Name} is missing ApiController metadata.");

            var versionAttributes = controllerType
                .GetCustomAttributes(typeof(ApiVersionAttribute), inherit: false)
                .Cast<ApiVersionAttribute>()
                .ToArray();
            Assert.Contains(versionAttributes, attribute =>
                attribute.Versions.Any(version => string.Equals(version.ToString(), "1.0", StringComparison.Ordinal)));

            var route = Assert.Single(controllerType
    ...
    {
        var actual = DiscoverPublicApiControllers()
        ...
        var expected = new[]
        {
            ...
            "POST api/v{version:apiVersion}/{gymCode}/payments/{id:guid}/refund",
            ...
            "PUT api/v{version:apiVersion}/{gymCode}/bookings/{id:guid}/reschedule",
            "PUT api/v{version:apiVersion}/{gymCode}/equipment-models/{id:guid}",
            "PUT api/v{version:apiVersion}/{gymCode}/equipment/{id:guid}",
            "PUT api/v{version:apiVersion}/{gymCode}/equipment/{id:guid}/status",
            ...
            "PUT api/v{version:apiVersion}/{gymCode}/training-sessions/{id:guid}/reschedule",
            "PUT api/v{version:apiVersion}/{gymCode}/training-sessions/{id:guid}/status",
            "PUT api/v{version:apiVersion}/{gymCode}/training-sessions/{id:guid}/trainer",
    ...
    private static bool HasApiControllerMetadata(Type controllerType)
    {
        return EnumerateControllerHierarchy(controllerType)
            .Any(type => type.GetCustomAttributes(typeof(ApiControllerAttribute), inherit: false).Any());
    }
