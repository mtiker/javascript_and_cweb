...
public class MemberWorkflowServiceTests
{
    ...
    [Fact]
    public async Task CreateMemberAsync_AutoGeneratesMemberCode_WhenRequestCodeIsEmpty()
    {
        var (dbContext, gymId) = await NewContextAsync();
        var service = CreateService(dbContext, gymId);

        var first = await service.CreateMemberAsync("test-gym", new MemberUpsertRequest
        {
            FirstName = "Pat",
            LastName = "Riley",
            MemberCode = "",
            PersonalCode = "49901040004",
            Status = MemberStatus.Active
        });

        var second = await service.CreateMemberAsync("test-gym", new MemberUpsertRequest
        {
            FirstName = "Sam",
            LastName = "Hughes",
            MemberCode = null!,
            PersonalCode = "49901040005",
            Status = MemberStatus.Active
        });

        Assert.Equal("M-TEST-GYM-0001", first.MemberCode);
        Assert.Equal("M-TEST-GYM-0002", second.MemberCode);
    }

    [Fact]
    public async Task CreateMemberAsync_AutoGeneratedCode_ContinuesFromHighestExistingSequence()
    {
        var (dbContext, gymId) = await NewContextAsync();
        SeedExistingMember(dbContext, gymId, memberCode: "M-TEST-GYM-0007", personalCode: "49901050001");
        await dbContext.SaveChangesAsync();
        ...
        var detail = await service.CreateMemberAsync("test-gym", new MemberUpsertRequest
        {
            ...
            LastName = "Klein",
            MemberCode = "   ",
            PersonalCode = "49901050002",
            Status = MemberStatus.Active
        });

        Assert.Equal("M-TEST-GYM-0008", detail.MemberCode);
    }
