...
public class TrainingWorkflowServiceTests
{
        ...
        IAuthorizationService? authorization = null)
    {
        ...
        return new TrainingWorkflowService(
            new EfTrainingPersistenceContext(dbContext),
            new EfTrainingCategoryRepository(dbContext),
            new EfTrainingSessionRepository(dbContext),
            new EfBookingRepository(dbContext),
            new TestGymsModuleApi(dbContext),
            new TestTrainingModuleApi(dbContext),
            new TestMembershipsModuleApi(dbContext),
    ...
    {
        public Task<GymAccess?> ResolveAccessAsync(
            Guid userId,
            string gymCode,
            IReadOnlyCollection<string>? allowedRoles = null,
            CancellationToken cancellationToken = default) =>
            Task.FromResult<GymAccess?>(null);

        public Task<IReadOnlyList<GymAccess>> ListGymsForUserAsync(
            Guid userId,
            CancellationToken cancellationToken = default) =>
            Task.FromResult<IReadOnlyList<GymAccess>>(Array.Empty<GymAccess>());

        public async Task<GymSettingsSummary?> GetSettingsAsync(
            Guid gymId,
            CancellationToken cancellationToken = default)
        {
            var settings = await dbContext.GymSettings
                .AsNoTracking()
                .FirstOrDefaultAsync(entity => entity.GymId == gymId, cancellationToken);

            return settings is null
                ? null
                : new GymSettingsSummary(settings.GymId, settings.CurrencyCode, settings.AllowNonMemberBookings);
        }
    }
    ...
    private sealed class TestMembershipsModuleApi(AppDbContext dbContext) : IMembershipsModuleApi
    {
            ...
            CancellationToken cancellationToken = default)
        {
            var member = await dbContext.Members
                .Include(entity => entity.Person)
                .FirstOrDefaultAsync(
                    entity => entity.GymId == gymId && entity.Id == memberId,
                    cancellationToken);
        ...
        public Task<MemberSummary?> FindMemberForUserAsync(
            Guid gymId,
            Guid userId,
            CancellationToken cancellationToken = default) =>
            Task.FromResult<MemberSummary?>(null);
    }
    ...
    private sealed class TestAuthorizationService(Guid gymId) : IAuthorizationService
    {
        ...
        public Task EnsureBookingAccessAsync(Guid gymIdValue, Guid memberId, Guid trainingSessionId, CancellationToken cancellationToken = default)
        {
            if (EnsureBookingAccessThrows)
            {
                throw new ForbiddenException("Booking access denied.");
    ...
    private sealed class TestUserContextService : IUserContextService
    {
        public UserExecutionContext GetCurrent() => new(
            ...
            ActiveRole: SharedKernel.RoleNames.GymAdmin,
            AllRoles: new[] { SharedKernel.RoleNames.GymAdmin },
