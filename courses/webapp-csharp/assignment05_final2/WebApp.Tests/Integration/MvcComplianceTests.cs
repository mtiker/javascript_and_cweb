...
public class MvcComplianceTests(CustomWebApplicationFactory factory) : IClassFixture<CustomWebApplicationFactory>
{
    ...
    [
        ...
        "/Admin/Memberships",
        "/Admin/MembershipPackages",
        "/Admin/MembershipPackages/Create",
        "/Admin/Sessions",
        "/Admin/Operations",
        "/Admin/TrainingCategories",
        "/Admin/TrainingCategories/Create"
    ];
    ...
    public async Task AnonymousUser_CannotAccess_AdminRoute(string route)
    {
        var client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
            AllowAutoRedirect = false
    ...
    [Fact]
    public void AdminControllers_AreProtectedAndAvoidLegacyInfrastructure()
    {
        var adminControllersRoot = Path.Combine(ResolveAssignmentRoot(), "WebApp", "Areas", "Admin", "Controllers");
        var controllerFiles = Directory.GetFiles(adminControllersRoot, "*Controller.cs", SearchOption.AllDirectories);
        ...
        foreach (var file in controllerFiles)
        {
            ...
            Assert.DoesNotContain("using App.BLL;", source);
            Assert.DoesNotContain("using App.DAL", source);
            Assert.DoesNotContain("AppDbContext", source);
            Assert.DoesNotContain("IAppDbContext", source);
            Assert.DoesNotContain("DbContext", source);
        }
    }

    [Fact]
    public void AdminViewsAndControllers_DoNotUseViewBagOrViewData()
    {
        var adminAreaRoot = Path.Combine(ResolveAssignmentRoot(), "WebApp", "Areas", "Admin");
        var files = Directory.GetFiles(adminAreaRoot, "*.*", SearchOption.AllDirectories)
    ...
    public static IEnumerable<object[]> AdminRoutes() =>
        TenantAdminRoutes.Select(route => new object[] { route });
}
