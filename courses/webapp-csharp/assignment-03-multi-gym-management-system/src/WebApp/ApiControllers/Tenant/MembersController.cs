...
public class MembersController(IMediator mediator) : ApiControllerBase
{
    [HttpGet]
    ...
    {
        return Ok(await mediator.SendAsync(new ListMembersQuery(gymCode), cancellationToken));
    }
    ...
    {
        return Ok(await mediator.SendAsync(new GetCurrentMemberQuery(gymCode), cancellationToken));
    }
    ...
    {
        return Ok(await mediator.SendAsync(new GetMemberQuery(gymCode, id), cancellationToken));
    }
    ...
    {
        var member = await mediator.SendAsync(new CreateMemberCommand(gymCode, request), cancellationToken);
    ...
    {
        return Ok(await mediator.SendAsync(new UpdateMemberCommand(gymCode, id, request), cancellationToken));
    }
    ...
    {
        await mediator.SendAsync(new DeleteMemberCommand(gymCode, id), cancellationToken);
        return NoContent();
    }
}
