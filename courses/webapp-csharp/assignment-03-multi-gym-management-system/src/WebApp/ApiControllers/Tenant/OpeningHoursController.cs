...
public class OpeningHoursController(IMediator mediator) : ApiControllerBase
{
    ...
    {
        return Ok(await mediator.SendAsync(new ListOpeningHoursQuery(gymCode), cancellationToken));
    }
    ...
    {
        return Ok(await mediator.SendAsync(new CreateOpeningHoursCommand(gymCode, request), cancellationToken));
    }
    ...
    {
        return Ok(await mediator.SendAsync(new UpdateOpeningHoursCommand(gymCode, id, request), cancellationToken));
    }
    ...
    {
        await mediator.SendAsync(new DeleteOpeningHoursCommand(gymCode, id), cancellationToken);
