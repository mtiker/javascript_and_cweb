...
public class GymUsersController(IMediator mediator) : ApiControllerBase
{
    [HttpGet("gym-users")]
    ...
    {
        return Ok(await mediator.SendAsync(new ListGymUsersQuery(gymCode), cancellationToken));
    }
    ...
    {
        return Ok(await mediator.SendAsync(new UpsertGymUserCommand(gymCode, request), cancellationToken));
    }
    ...
    {
        await mediator.SendAsync(new DeleteGymUserCommand(gymCode, appUserId, roleName), cancellationToken);
