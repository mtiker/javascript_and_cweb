...
public class Final1PresentationBoundaryTests
{
    ...
    public void MvcControllersAndViewComponents_DoNotInjectAppDbContext()
    {
        ...
        var files = Directory
            .GetFiles(Path.Combine(root, "src", "WebApp"), "*.cs", SearchOption.AllDirectories)
            .Where(path =>
    ...
    public void AdminAndClientPageServices_DoNotInjectConcreteAppDbContext()
    {
        ...
        {
            Path.Combine(root, "src", "WebApp", "Areas", "Admin", "Services"),
            Path.Combine(root, "src", "WebApp", "Areas", "Client", "Services")
        };
